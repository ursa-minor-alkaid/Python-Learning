...
- **“语法糖”（Syntactic Sugar）**
  - **语法糖**是指在编程语言中添加的某种语法，这种语法对语言的功能没有影响，但能让程序员更方便、更清晰地使用语言。
    - 它是一种“更甜美”的书写方式，其背后对应着更基础、更复杂的代码结构。
    - 换句话说，语法糖让代码写起来更简单，读起来更清晰，但它并没有创造任何新的计算机能力。**解释器或编译器会负责将其“脱糖（Desugar）”，翻译成它背后等价的、更底层的形式。**
  - Python中有非常多的语法糖，比如，`+` 运算符就是 `__add__` 方法的语法糖
  - 这意味着，Python 很多高级特性的底层，其实都能用更基础的语法实现出来。所以理论上，同一段逻辑往往存在：
    - **简洁、地道（Pythonic）的写法** —— 推荐日常使用
    - **更基础、更显式、更“原始”的写法** —— 理解原理时有用，实际项目中较少直接写
